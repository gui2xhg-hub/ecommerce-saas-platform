...
// FUNÇÃO AUXILIAR PARA PARSE SEGURO DE VALORES MONETÁRIOS E NÚMEROS
...
export default function AdminTenant() {
  ...
  // CONFIGURAÇÕES DE FRETE E AVISOS
  ...
  // FORMULÁRIOS E EDIÇÃO DE PRODUTOS
  ...
  // ESTADOS TEMPORÁRIOS PARA VARIAÇÕES E GALERIA
  ...
  // FILTRO E BUSCA DE CLIENTES NO CRM
  ...
  const fetchTenant = async () => {
    ...
    if (tData) {
      ...
      setAnnouncementBar(tData.custom_message || '');
    }
    setLoading(false);
  };
  ...
  const handleSaveTenantSettings = async (e) => {
    ...
    const updatePayload = {
      ...
      custom_message: announcementBar,
  ...
  // MANIPULAÇÃO DE IMAGENS E VARIAÇÕES
  ...
  const handleRemoveImageFromNewProd = (index) => {
    setNewProd(prev => {
      ...
      return { ...prev, image: updated.length > 0 ? updated[0] : '', images_json: updated };
    });
  };
  ...
  const handleAddVariationToNewProd = () => {
    if (!tempVarName.trim() || !tempVarOptions.trim()) return alert("Preencha o nome e as opções!");
    const optsArray = tempVarOptions.split(',').map(s => s.trim()).filter(Boolean);
    setNewProd(prev => ({ ...prev, variations_json: [...(prev.variations_json || []), { attribute_name: tempVarName.trim(), options: optsArray }] }));
    setTempVarName('');
    setTempVarOptions('');
  };

  const handleRemoveVariationFromNewProd = (index) => {
    setNewProd(prev => ({ ...prev, variations_json: (prev.variations_json || []).filter((_, i) => i !== index) }));
  };

  const handleAddVariationToEditProd = () => {
    if (!tempEditVarName.trim() || !tempEditVarOptions.trim()) return alert("Preencha o nome e as opções!");
    const optsArray = tempEditVarOptions.split(',').map(s => s.trim()).filter(Boolean);
    setEditingProduct(prev => ({ ...prev, variations_json: [...(prev.variations_json || []), { attribute_name: tempEditVarName.trim(), options: optsArray }] }));
    setTempEditVarName('');
    setTempEditVarOptions('');
  };

  const handleRemoveVariationFromEditProd = (index) => {
    setEditingProduct(prev => ({ ...prev, variations_json: (prev.variations_json || []).filter((_, i) => i !== index) }));
  };

  // CRUD PRODUTOS COM SUPORTE A ESTOQUE
  const handleAddProduct = async (e) => {
    ...
    if (!targetCategoryId || isNaN(targetCategoryId)) return alert("Selecione uma categoria válida!");
    ...
    const stockQty = newProd.stock !== '' ? parseInt(newProd.stock) : null;
    const isActive = stockQty !== null ? stockQty > 0 : true;
  ...
  const handleUpdateProduct = async (e) => {
    ...
    const stockQty = editingProduct.stock !== '' && editingProduct.stock !== null ? parseInt(editingProduct.stock) : null;
    const autoActive = stockQty !== null ? stockQty > 0 : editingProduct.active;
  ...
  const handleAddCoupon = async (e) => {
    e.preventDefault();
    if (!newCoupon.code || !newCoupon.discount_value) return alert("Preencha o código e o valor!");
    ...
    const { error } = await supabase.from('coupons').insert([{
      ...
      discount_value: parsePrice(newCoupon.discount_value),
      active: true
    }]);
  ...
  const handleAddNeighborhood = async (e) => {
    e.preventDefault();
    await supabase.from('neighborhoods').insert([{ tenant_id: tenant.id, name: newNeigh.name.trim(), fee: parsePrice(newNeigh.fee) }]);
  ...
  const handleUpdateNeigh = async (e) => {
    e.preventDefault();
    await supabase.from('neighborhoods').update({ name: editingNeigh.name.trim(), fee: parsePrice(editingNeigh.fee) }).eq('id', editingNeigh.id);
  ...
  // CÁLCULOS DOS RELATÓRIOS E FINANCEIRO
  ...
  // AGRUPAMENTO DE CLIENTES PARA O CRM
  const getCustomersList = () => {
    const customerMap = {};

    allOrders.forEach(ord => {
      const phoneClean = ord.customer_phone ? ord.customer_phone.replace(/\D/g, '') : 'SEM_FONE';
      if (!phoneClean || phoneClean === 'SEM_FONE') return;
    ...
    <div className="min-h-screen bg-gray-950 text-white p-4 max-w-4xl mx-auto font-sans pb-16">
      <header className="flex justify-between items-center py-4 border-b border-gray-800 mb-6 print:hidden">
        <div>
          ...
          <p className="text-xs text-gray-400">Painel Administrativo & Gestão de Vendas</p>
        </div>
      ...
      <div className="flex space-x-1 bg-gray-900 p-1.5 rounded-2xl border border-gray-800 mb-6 text-[11px] font-bold overflow-x-auto print:hidden">
        {[
          ...
          { id: 'shipping', label: '🚚 Fretes & Envios' },
          { id: 'crm', label: '👥 Clientes VIP' },
          { id: 'reports', label: '📊 Financeiro' },
          { id: 'settings', label: '⚙️ Configurações' },
        ].map(tab => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id)}
            style={activeTab === tab.id ? { backgroundColor: primaryColor, color: '#FFFFFF' } : {}}
            className={`flex-1 py-2.5 px-3 rounded-xl whitespace-nowrap transition ${activeTab === tab.id ? 'shadow-md' : 'text-gray-400 hover:text-white'}`}>
            {tab.label}
          </button>
        ))}
      </div>
        ...
        <div className="space-y-6">
          <section className="bg-gray-900 p-5 rounded-3xl border border-gray-800 space-y-4 shadow-xl">
            ...
            <form onSubmit={handleAddProduct} className="space-y-3">
              ...
              <input type="text" placeholder="Descrição detalhada" value={newProd.description} className="w-full bg-gray-950 border border-gray-800 p-3 rounded-xl text-xs text-white focus:outline-none" onChange={(e) => setNewProd({ ...newProd, description: e.target.value })} />
              
              <div className="grid grid-cols-4 gap-2">
                <div>
                  <label className="text-[10px] text-gray-400 block mb-1">Preço Normal (R$):</label>
                  <input type="text" placeholder="Preço" value={newProd.price} className="w-full bg-gray-950 border border-gray-800 p-3 rounded-xl text-xs text-white focus:outline-none" onChange={(e) => setNewProd({ ...newProd, price: e.target.value })} />
                </div>
              ...
              {/* PESO E DIMENSÕES PARA CORREIOS */}
              <div className="bg-gray-950 p-3.5 rounded-2xl border border-gray-800 space-y-2">
                <label className="text-xs font-bold text-orange-400 block">📦 Peso e Dimensões (Para Frete por CEP / Correios)</label>
              ...
              <div className="bg-gray-950 p-3.5 rounded-2xl border border-gray-800 space-y-3">
                ...
                <div className="flex space-x-2">
                  ...
                  />
                  <button type="button" onClick={handleAddImageToNewProd} className="bg-blue-600 hover:bg-blue-700 text-white font-bold px-3 py-2 rounded-xl text-xs transition">+ Foto</button>
                </div>
                  ...
                  <div className="flex flex-wrap gap-2 pt-2 border-t border-gray-800">
                    {newProd.images_json.map((imgUrl, idx) => (
                      <div key={idx} className="relative group w-16 h-16 rounded-xl border border-gray-800 overflow-hidden bg-gray-900">
                        ...
                        <button type="button" onClick={() => handleRemoveImageFromNewProd(idx)} className="absolute top-0.5 right-0.5 bg-red-600 text-white rounded-full w-4 h-4 text-[10px] flex items-center justify-center font-bold">✕</button>
                      </div>
                    ))}
                  </div>
                )}
              ...
              {/* VARIAÇÕES */}
              <div className="bg-gray-950 p-3.5 rounded-2xl border border-gray-800 space-y-3">
                <label className="text-xs font-bold text-blue-400 block">⚡ Variações / Opções (Ex: Tamanho, Cor, Voltagem)</label>
                <div className="flex space-x-2">
                  <input type="text" placeholder="Atributo (ex: Tamanho)" value={tempVarName} onChange={(e) => setTempVarName(e.target.value)} className="w-1/2 bg-gray-900 border border-gray-800 p-2.5 rounded-xl text-xs text-white focus:outline-none" />
                  <input type="text" placeholder="Opções separadas por vírgula (ex: P, M, G)" value={tempVarOptions} onChange={(e) => setTempVarOptions(e.target.value)} className="w-1/2 bg-gray-900 border border-gray-800 p-2.5 rounded-xl text-xs text-white focus:outline-none" />
                </div>
                <button type="button" onClick={handleAddVariationToNewProd} className="w-full bg-gray-800 hover:bg-gray-700 text-blue-400 font-bold py-2 rounded-xl text-xs border border-gray-700 transition">+ Adicionar Variação</button>
              ...
              <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 font-bold py-3.5 rounded-xl text-xs text-white transition shadow-lg">Salvar Produto 🚀</button>
            </form>
          </section>
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <div className="flex items-center space-x-2">
                        ...
                        {hasPromo && <span className="bg-red-500/20 text-red-400 border border-red-500/30 text-[9px] font-bold px-1.5 py-0.5 rounded-md">🔥 Promo</span>}
      ...
      {/* ABA DE FRETES & ENVIOS (REORGANIZADA E SIMPLIFICADA) */}
      {activeTab === 'shipping' && (
        <div className="space-y-6">
          <section className="bg-gray-900 p-5 rounded-3xl border border-orange-500/30 space-y-4 shadow-xl">
            <h3 className="font-bold text-sm text-orange-400 flex items-center space-x-2">
              <span>🚚 Modo de Operação de Entregas</span>
            </h3>

            <div className="space-y-4">
              <div>
                <label className="text-xs font-bold text-blue-400 block mb-1">Selecione como a loja calcula o valor da entrega:</label>
                <select
                  value={shippingMode}
                  ...
                  <option value="local">🛵 Apenas Entrega Local (Bairros da Cidade ou Taxa Fixa)</option>
                  <option value="national">📦 Apenas Envio Nacional por CEP (Correios / Transportadoras)</option>
                  <option value="hybrid">⚡ Híbrido (Entrega Local na cidade + Envio via CEP para todo o Brasil)</option>
                </select>
              </div>

              {/* OPÇÃO 1: FRETES LOCAIS */}
              {(shippingMode === 'local' || shippingMode === 'hybrid') && (
                <div className="space-y-4 bg-gray-950 p-4 rounded-2xl border border-gray-800">
                  <h4 className="font-bold text-xs text-blue-400">🛵 Configuração de Entrega Local / Motoboy</h4>
                  
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-xs text-gray-400 block mb-1">Taxa Padrão Local (R$):</label>
                      <input
                        type="number"
                        step="0.01"
                      ...
                      />
                      <span className="text-[9px] text-gray-500 block mt-0.5">Usada se o bairro não estiver cadastrado.</span>
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-gray-900 border border-gray-800 p-2.5 rounded-xl text-xs text-white focus:outline-none"
                      />
                      <span className="text-[9px] text-gray-500 block mt-0.5">(0 = Sem frete grátis automático)</span>
                    </div>
                  </div>

                  <div>
                    <label className="text-xs text-gray-400 block mb-1">Permitir Retirada no Balcão / Loja Física?</label>
                    <select
                      value={enablePickup ? 'SIM' : 'NAO'}
                      onChange={(e) => setEnablePickup(e.target.value === 'SIM')}
                      className="w-full bg-gray-900 border border-gray-800 p-2.5 rounded-xl text-xs text-white focus:outline-none">
                      <option value="SIM">Sim (Cliente pode escolher Retirar Grátis)</option>
                      <option value="NAO">Não (Apenas Entrega)</option>
                    </select>
                  </div>

                  <div className="pt-3 border-t border-gray-800 space-y-3">
                    <h5 className="font-bold text-xs text-gray-300">📍 Cadastrar Taxa por Bairro / Região</h5>
                    <form onSubmit={handleAddNeighborhood} className="flex space-x-2">
                      <input type="text" placeholder="Nome do Bairro (Ex: Centro)" value={newNeigh.name} onChange={(e) => setNewNeigh({ ...newNeigh, name: e.target.value })} className="w-1/2 bg-gray-900 border border-gray-800 p-2.5 rounded-xl text-xs text-white focus:outline-none" />
                      <input type="text" placeholder="Taxa R$ (Ex: 8.00)" value={newNeigh.fee} onChange={(e) => setNewNeigh({ ...newNeigh, fee: e.target.value })} className="w-1/4 bg-gray-900 border border-gray-800 p-2.5 rounded-xl text-xs text-white focus:outline-none" />
                      <button type="submit" className="w-1/4 bg-green-600 hover:bg-green-700 text-white font-bold py-2 rounded-xl text-xs transition">+ Bairro</button>
                    </form>

                    <div className="space-y-1.5 max-h-40 overflow-y-auto">
                      {neighborhoods.map((n) => (
                        <div key={n.id} className="bg-gray-900 p-2.5 rounded-xl border border-gray-800 flex justify-between items-center text-xs">
                          <span><b>{n.name}:</b> <span className="text-green-400">R$ {Number(n.fee).toFixed(2)}</span></span>
                          <button onClick={async () => { await supabase.from('neighborhoods').delete().eq('id', n.id); fetchData(); }} className="text-red-400 font-bold px-2">🗑</button>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              {/* OPÇÃO 2: FRETE NACIONAL POR CEP */}
              {(shippingMode === 'national' || shippingMode === 'hybrid') && (
                <div className="space-y-3 bg-gray-950 p-4 rounded-2xl border border-gray-800">
                  <h4 className="font-bold text-xs text-orange-400">📦 Parâmetros para Cálculo Automático por CEP</h4>
                  
                  <div>
                    <label className="text-xs text-gray-400 block mb-1">CEP de Origem da Loja (Saída da encomenda):</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="text-xs text-gray-400 block mb-1">Token Melhor Envio (Opcional):</label>
                    <input
                      type="password"
                      placeholder="Token API para cotação direta no seu painel do Melhor Envio"
              ...
              <button
                ...
                className="w-full bg-orange-600 hover:bg-orange-700 text-white font-bold py-3.5 rounded-xl text-xs transition shadow-md">
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center bg-gray-900 p-4 rounded-2xl border border-gray-800 text-xs print:hidden">
            <div className="flex items-center space-x-2">
              ...
              <select value={reportFilter} onChange={(e) => setReportFilter(e.target.value)} className="bg-gray-950 border border-gray-800 p-2 rounded-xl text-white font-bold">
            ...
            <button
              ...
              className="bg-blue-600 hover:bg-blue-700 text-white font-bold px-4 py-2 rounded-xl text-xs transition shadow flex items-center space-x-1">
          ...
          <section className="bg-gray-900 p-5 rounded-3xl border border-gray-800 space-y-3 shadow-xl">
            <h3 className="font-bold text-xs text-blue-400 uppercase tracking-wider">🏆 PRODUTOS MAIS VENDIDOS</h3>
            <div className="space-y-2">
              {topProducts.length === 0 ? (
                <p className="text-xs text-gray-400">Nenhum pedido pago no período.</p>
              ) : (
                topProducts.map((p, idx) => (
                  <div key={idx} className="flex justify-between items-center bg-gray-950 p-3 rounded-xl text-xs border border-gray-800">
                    ...
                    <span className="bg-blue-500/20 text-blue-400 border border-blue-500/30 px-3 py-1 rounded-lg font-bold">{p.qty} un. vendidas</span>
                  </div>
                ))
              )}
            </div>
          ...
          <section className="bg-gray-900 p-4 rounded-2xl border border-red-500/30 flex justify-between items-center mt-4 print:hidden">
            <div>
              <h4 className="font-bold text-xs text-red-400">🧹 Zerar Dados de Vendas</h4>
              <p className="text-[10px] text-gray-400">Apaga todo o histórico de pedidos para testes.</p>
            </div>
            <button type="button" onClick={handleClearFinancialData} className="bg-red-500/20 hover:bg-red-500/40 text-red-400 border border-red-500/40 px-3 py-2 rounded-xl text-xs font-bold transition">🗑️ Limpar</button>
          </section>
        </div>
      )}
        ...
        <div className="space-y-6">
          <section className="bg-gray-900 p-5 rounded-3xl border border-gray-800 space-y-4 shadow-xl">
            ...
            <form onSubmit={handleAddCategory} className="flex space-x-2">
              <input type="text" placeholder="Nome (Ex: Lançamentos, Skincare, Eletrônicos)" value={newCatName} className="flex-1 bg-gray-950 border border-gray-800 p-3 rounded-xl text-xs text-white focus:outline-none" onChange={(e) => setNewCatName(e.target.value)} />
          ...
          <section className="space-y-2">
            {categories.map((c) => (
              <div key={c.id} className="bg-gray-900 p-3.5 rounded-2xl border border-gray-800 flex justify-between items-center text-xs">
                ...
                <button onClick={async () => { if (confirm("Excluir categoria?")) { await supabase.from('categories').delete().eq('id', c.id); fetchData(); } }} className="text-red-400 font-bold p-1">🗑 Excluir</button>
              </div>
            ))}
          </section>
        </div>
        ...
        <div className="space-y-6">
          <section className="bg-gray-900 p-5 rounded-3xl border border-gray-800 space-y-4 shadow-xl">
            ...
            <form onSubmit={handleAddCoupon} className="space-y-3">
              <input
                type="text"
                placeholder="Código do Cupom (Ex: PRIMEIRA10)"
              ...
              <div className="flex space-x-2">
                ...
                <input
                  type="text"
                  placeholder={newCoupon.discount_type === 'percent' ? "Ex: 10 (%)" : "Ex: 15.00 (R$)"}
                  value={newCoupon.discount_value}
                  onChange={(e) => setNewCoupon({ ...newCoupon, discount_value: e.target.value })}
                  className="w-1/2 bg-gray-950 border border-gray-800 p-3 rounded-xl text-xs text-white focus:outline-none"
                />
              </div>
              <button type="submit" className="w-full bg-green-600 hover:bg-green-700 font-bold py-3 rounded-xl text-xs text-white transition">Cadastrar Cupom 🎟️</button>
            </form>
          </section>
      ...
      {/* ABA DE CONFIGURAÇÕES GERAIS DA LOJA */}
      {activeTab === 'settings' && (
        <div className="space-y-6">
          <section className="bg-gray-900 p-5 rounded-3xl border border-gray-800 space-y-4 shadow-xl">
            <h3 className="font-bold text-sm text-blue-400">⚙️ Personalização & Avisos da Loja</h3>
            <form onSubmit={handleSaveTenantSettings} className="space-y-3">
              ...
              {/* BARRA DE AVISOS E PROMOÇÕES PERSONALIZADA */}
              <div>
                <label className="text-[11px] text-orange-400 font-bold block mb-1">📢 Barra de Aviso / Comunicado no Topo da Loja:</label>
                <input 
                  type="text" 
                  placeholder="Ex: 📦 Prazo de produção: 5 dias úteis | ⚡ Envio Rápido para todo o Brasil" 
                  value={announcementBar} 
                  onChange={(e) => setAnnouncementBar(e.target.value)} 
                  className="w-full bg-gray-950 border border-orange-500/40 p-3 rounded-xl text-xs text-white focus:outline-none" 
                />
              </div>

              <div>
                <label className="text-[11px] text-blue-400 font-bold block mb-1">🎯 Segmento / Nicho:</label>
                <select
                  ...
                  <option value="general">📦 E-commerce Geral</option>
                </select>
              </div>
              ...
              <div>
                ...
                <input type="text" value={tenant.instagram_url || ''} className="w-full bg-gray-950 border border-gray-800 p-3 rounded-xl text-xs text-white focus:outline-none" onChange={(e) => setTenant({ ...tenant, instagram_url: e.target.value })} />
              </div>

              <div>
                <label className="text-[11px] text-gray-400 block mb-1">Banners Promocionais da Vitrine (URLs separadas por vírgula):</label>
                <input type="text" value={tenant.promo_banners || ''} className="w-full bg-gray-950 border border-gray-800 p-3 rounded-xl text-xs text-white focus:outline-none" onChange={(e) => setTenant({ ...tenant, promo_banners: e.target.value })} />
              </div>
              ...
              <div className="pt-3 border-t border-gray-800 space-y-3">
                <div className="flex justify-between items-center">
                  <div>
                    <h4 className="font-bold text-xs text-green-400">⚡ PIX Dinâmico Mercado Pago</h4>
                    <p className="text-[10px] text-gray-400">Gera QR Code e dá baixa automática sozinho no pedido.</p>
                  </div>
                ...
                {tenant.pix_enabled && (
                  <div className="space-y-2 bg-gray-950 p-3.5 rounded-2xl border border-gray-800">
                    <label className="text-[11px] text-gray-400 block mb-1">Access Token do Mercado Pago:</label>
                    <input type="password" placeholder="APP_USR-xxxx-xxxx..." value={tenant.pix_access_token || ''} onChange={(e) => setTenant({ ...tenant, pix_access_token: e.target.value })} className="w-full bg-gray-900 border border-gray-800 p-2.5 rounded-xl text-xs text-white focus:outline-none font-mono" />
                  </div>
                )}
              </div>

              <button type="submit" className="w-full bg-green-600 hover:bg-green-700 font-bold py-3.5 rounded-xl text-xs text-white transition shadow-lg mt-2">Salvar Alterações</button>
            </form>
          </section>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <form onSubmit={handleUpdateProduct} className="bg-gray-900 w-full max-w-sm rounded-3xl p-5 border border-blue-500/40 space-y-3 max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <div className="grid grid-cols-3 gap-2">
              <div>
                <label className="text-[10px] text-gray-400 block mb-1">Preço Normal:</label>
                <input type="text" value={editingProduct.price} onChange={(e) => setEditingProduct({ ...editingProduct, price: e.target.value })} className="w-full bg-gray-950 border border-gray-800 p-2.5 rounded-xl text-xs text-white focus:outline-none" />
              </div>

              <div>
                <label className="text-[10px] text-red-400 font-bold block mb-1">Preço Antigo:</label>
                <input type="text" value={editingProduct.original_price || ''} onChange={(e) => setEditingProduct({ ...editingProduct, original_price: e.target.value })} className="w-full bg-gray-950 border border-red-500/30 p-2.5 rounded-xl text-xs text-white focus:outline-none" />
              </div>

              <div>
                <label className="text-[10px] text-green-400 font-bold block mb-1">Estoque:</label>
                <input type="number" value={editingProduct.stock ?? ''} onChange={(e) => setEditingProduct({ ...editingProduct, stock: e.target.value })} className="w-full bg-gray-950 border border-green-500/30 p-2.5 rounded-xl text-xs text-white focus:outline-none" placeholder="Ilimitado" />
              </div>
            </div>
